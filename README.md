...
ModuleWhat it doesdnsA/MX/NS/TXT/SOA + AXFR zone transfer attempt against each NSportsMulti-threaded socket scanner + nmap wrapper (--nmap)subsConcurrent subdomain 
brute-force, wordlist-swappablewebHTTP fingerprinting: tech stack, security header audit, cookie flagswhoisRegistrar, org, dates, emails via python-whois or socket
fallbackbannersService-specific probes (FTP, SSH, SMTP, Redis, HTTP, etc.)
