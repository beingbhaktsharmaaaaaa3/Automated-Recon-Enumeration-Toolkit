...
Examples:
  ...
  python3 main.py -t example.com --resume ./scan_state.json
    ...
    p.add_argument("-o", "--output", default=None,
                   help="Base path for output files (produces .json, .txt, .html)")
    p.add_argument("--json-only", action="store_true",
                   help="Save JSON report only")
...
def build_config(args) -> dict:
    ...
    return {
        ...
        "json_only":    args.json_only or cfg.get("json_only", False),
...
def main():
    ...
    reporter.save_json(out_base + ".json")
    if not cfg["json_only"]:
        reporter.save_text(out_base + ".txt")
    ...
    log("OK", f"Reports saved to: {out_base}.[json|txt|html]")
